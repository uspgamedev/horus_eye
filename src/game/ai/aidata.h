...
class AIData {
  public:
    ...
    const ugdk::math::Vector2D& direction() const { return direction_; }
    void set_direction(const ugdk::math::Vector2D& dir) { direction_ = dir; }

    const ugdk::math::Vector2D& aim_destination() const { return aim_destination_; }
    void set_aim_destination(const ugdk::math::Vector2D& aim) { aim_destination_ = aim; }
