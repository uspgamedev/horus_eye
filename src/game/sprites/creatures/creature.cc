...
void Creature::DoMove(float delta_t) {
	Move(this->GetWalkingDirection(), delta_t);
	this->SelectAnimation(walking_animations_[animation_direction_]);
}
void Creature::SetAI(ai::AI* new_ai) {
	if (ai_)
		ai_->Finish();
	ai_ = new_ai;
	ai_->Start();
...
void Creature::SetDirection(Vector2D direction) {
	Vector2D dir_animation = scene::World::FromWorldCoordinates(direction); 
	...
	walking_direction_ = Vector2D::Normalized(direction);
